// Config.js v3.4.1
	...
	var pzprEnv = require('../pzpr/env.js');
	var pzprVariety = require('../pzpr/variety.js');
	//---------------------------------------------------------------------------
	...
	Config.prototype = {
		...
		init: function() {
			...
			/* 入力方法設定 */
			this.add("use", !pzprEnv.API.touchevent ? 1 : 2, {
				option: [1, 2]
			}); /* 黒マスの入力方法 */
			this.add("use_tri", 1, {
				option: [1, 2, 3]
		...
		getNormalizedName: function(argname) {
			...
			if (argname.match(/\@/)) {
				...
				var pid = pzprVariety.toPID(splitted[1]);
				if (!!pid) {
					info.pid = pid;
				}
			}
